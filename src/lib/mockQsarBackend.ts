...
const API_BASE = import.meta.env.VITE_QSAR_API_BASE ?? "http://127.0.0.1:27051";
...
export async function runFilters(sessionId: string, settings: FilterSettings): Promise<DatasetProfile> {
...
export async function runValidations(sessionId: string, settings: ValidationSettings): Promise<ValidationResult> {
